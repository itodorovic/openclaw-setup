...
| AI Agent | OpenClaw native install via npm under ~/.local (systemd user service) |
...
This script fixes permissions, reinstalls OpenClaw under `~/.local` (user-writable npm prefix, required for GUI self-updates), installs Tailscale, and configures the gateway in one shot.
